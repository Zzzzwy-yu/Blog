<template>
  <div style="min-height:100vh;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:flex;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:12px;padding:40px;width:420px;box-shadow:0 12px 40px rgba(0,0,0,0.2);">
      <h1 style="margin:0 0 8px;text-align:center;color:#303133;">用户登录</h1>
      <p style="text-align:center;color:#909399;margin-bottom:28px;font-size:13px;">欢迎回来</p>
...
const login = () => {
  formRef.value.validate(valid => {
    ...
    userLogin(form).then(res => {
      ...
      userStore.setUserInfo({ username: res.username, nickname: res.nickname, avatar: res.avatar })
